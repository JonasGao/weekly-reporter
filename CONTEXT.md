...
紧邻当前周报日期范围的上一周期中，某一受众版本已采用且状态为 current 的终版所包含的下周计划。它是当前周报生成时可引用的历史参考，不是当前周报的事件事实。
...
时间线页面右侧边栏中只读展示的计划事项投影，来源是上一周期各受众版本已采用的 current 终版。它保留事项文本、来源周期、受众和来源状态等审计信息，不承载负责人、日期、优先级或完成状态。
_Avoid_: 任务列表、计划库
...
提供给终版生成会话的只读 AI 能力：查询符合条件的历史周报列表，并按周报与受众标识读取已采用终版内容。查询结果是带有受众和状态标记的历史参考上下文，不能取代当前受众版本的周报原稿作为事实来源。
...
公开过程记录中的**公开生成摘要 (Public Generation Summary)** 是每次完整提案轮次的稳定、可审计结论：包括计划判断与用户覆盖、历史参考元数据、工具状态、失败/截断状态和提案变更摘要。未发生的项目用明确的空值表示；它不复制历史正文、不推断隐藏推理，也不成为本周事实或评分输入。
