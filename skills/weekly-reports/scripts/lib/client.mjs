// Shared HTTP helpers for the weekly-reports skill scripts.
// Every script talks to the same local weekly-reporter server, so keeping the
// request + error handling in one place means a change to the failure messages
// (the part agents and humans actually read) only has to happen once.
...
export function requireId(raw, usage) {
  const id = Number.parseInt(raw ?? '', 10)
  if (!Number.isInteger(id)) fail(`Error: a numeric report ID is required, got '${raw ?? ''}'`, usage)
  return id
}
...
export function requireDate(value, flag) {
  if (!value || !DATE_RE.test(value)) fail(`Error: ${flag} must be a YYYY-MM-DD date, got '${value ?? ''}'`)
  return value
}

export function requireVariant(value) {
  if (value !== 'leadership' && value !== 'personal') {
    fail(`Error: --variant must be 'leadership' or 'personal', got '${value ?? ''}'`)
  }
  return value
}
